...
#2.0 - это рабочая версия с кнопкой для отправки команды на взлом пароля.
...
class Window(QtWidgets.QMainWindow):
    def __init__(self):#Конструктор
        ...
        uic.loadUi('Touch_screen_Controller_v2.0.ui', self) #Загружаем наш интерфейс, ктр создали в QTDesigner 
        ...
        self.PSWD_hack_btn.clicked.connect(self.onPSWD_hack_btnclick)
    ...
    def onPSWD_hack_btnclick(self):
         #Команда 50 - Перебрать пароль, начиная от N=Ц1Ц2Ц3Ц4 (Ц - это цифра)
         #            Команда,Ц1Ц2,Ц3Ц4,00
        #command = bytes([0x50,0x00,0x00,0x00])
        command = bytes([0x50,0x21,0x51,0x00])
        self.serial.write(command)
        self.plainTextEditLog.appendPlainText("Послали команду на взлом пароля")
    ...
    def onNumbtnclick(self):#Обработчик нажатий на кнопки 0-9
        ...
        #Остальное можно вынести в отдельный метод SendTouch и переиспользовать его в нужных местах
        #А здесь просто вызывать этот метод и передавть ему текст кнопки.
